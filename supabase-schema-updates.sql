...
-- First, let's check if the regions table exists and see its current structure
    ...
    LOOP
        BEGIN
            ...
            INSERT INTO regions (
                ...
                COALESCE(region_id, 'region-' || extract(epoch from now()) || '-' || floor(random() * 1000)),
                region_name,
                region_subtitle,
                region_pos_x,
                region_pos_y,
            ...
            -- Return success
            inserted_id := COALESCE(region_id, 'region-' || extract(epoch from now()));
            success := true;
            error_message := NULL;
            RETURN NEXT;
...
CREATE OR REPLACE FUNCTION bulk_insert_places(places_data JSONB, parent_region_id TEXT)
RETURNS TABLE (
    inserted_id TEXT,
    success BOOLEAN,
    error_message TEXT
...
-- 13. EXAMPLE QUERIES TO TEST THE SETUP
...
-- Bulk insert regions from JSON (example usage)
-- SELECT * FROM bulk_insert_regions('[{"id": "test-region", "name": "Test Region", "subtitle": "Test", "position": {"x": 50, "y": 50}, "color": "from-blue-500 to-blue-600", "description": "Test description", "keyLocations": ["Location 1"], "population": "1000", "threat": "Low", "visibility": {"freeUsers": true, "signedInUsers": true, "premiumUsers": true}}]'::jsonb);
...
GRANT EXECUTE ON FUNCTION bulk_insert_places(JSONB, TEXT) TO authenticated;
